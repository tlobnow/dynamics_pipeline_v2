...
file_list <- file_list %>% 
  mutate(protein_path        = file.path(tracking_path, protein_relative_path)) %>% 
  mutate(image               = paste0(protein_path, new_image_ending),
  ...
  mutate(xml = paste0(xml, ".xml")) %>% 
  # Keep only files that exist
  filter(file.exists(image), file.exists(xml)) %>% 
  as.data.table()
...
MissingSpotsFx <- function(TableX){
  tryCatch({
    ...
    MissingFrames <- MissingFrames %>% 
      arrange(FRAME) %>% 
      mutate(PREVIOUS_POSITION_X  = POSITION_X,
              NEXT_POSITION_X     = POSITION_X,
              PREVIOUS_POSITION_Y = POSITION_Y,
              NEXT_POSITION_Y     = POSITION_Y) %>% 
      fill(PREVIOUS_POSITION_X, PREVIOUS_POSITION_Y, .direction = "down") %>% 
      fill(NEXT_POSITION_X, NEXT_POSITION_Y, .direction = "up") %>% 
      filter(MISSING == TRUE) %>% 
      group_by(PREVIOUS_POSITION_X, PREVIOUS_POSITION_Y, NEXT_POSITION_X, NEXT_POSITION_Y) %>% 
      mutate(POSITION    = 1:n() / (n() + 1),
      ...
      ungroup() %>% select(TRACK_ID, FRAME, POSITION_X, POSITION_Y)
...
XMLtoTableFx <- function(FileX){
  tryCatch({print(paste("XMLtoTableFx FileX =", FileX))
    temp_file_list <- file_list %>% filter(xml == xml_list[FileX]) %>% as.data.table()
    # Get cell data
    ...
    temp_file_list <- temp_file_list%>% 
      as_tibble() %>%
      mutate(RELATIVE_PATH = PROTEIN_RELATIVE_PATH,
              ...
              CELL_AREA       = AREA) %>% 
      select(-c(PROTEIN_RELATIVE_PATH, PROTEIN_NAME, POSITION_X, POSITION_Y, AREA, INTENSITY_OUTPUT, COORDINATES_OUTPUT, XML)) 
    ...
    SPOTS1 <- TracksTable %>%
      mutate(ID = SPOT_SOURCE_ID) %>%
      select(ID, TRACK_ID) %>% 
      as.data.table()
    
    SPOTS2 <- TracksTable %>%
      mutate(ID = SPOT_TARGET_ID) %>%
      select(ID, TRACK_ID) %>% 
      as.data.table()
    ...
    MissingSpotsTables <- SpotsTable %>% 
      group_by(TRACK_ID) %>% 
      mutate(RANGE  = max(FRAME) - min(FRAME), N = n() - 1) %>% 
      filter(RANGE != N) %>% 
      ungroup() %>% 
      select( TRACK_ID, FRAME, POSITION_X, POSITION_Y) %>% 
      arrange(TRACK_ID, FRAME) %>% as_tibble() %>% group_split(TRACK_ID)
    ...
    select_cell_parameters <- temp_file_list %>% select(IMAGE, CELL, PROTEIN) %>% as.data.table()
    ...
    BindProteinData <- function(p, SpotsTable, select_cell_parameters){
      ...
      TempShortFullTable <- TempShortFullTable %>% 
        select(IMAGE, CELL, PROTEIN, TRACK_ID, QUALITY, POSITION_X, POSITION_Y, RADIUS, FRAME, contains("SNR")) %>% 
        mutate(UNIVERSAL_TRACK_ID             = paste(IMAGE, CELL, PROTEIN, TRACK_ID, sep = "..."),
               UNIVERSAL_COLOCALIZATION_GROUP = paste(IMAGE, CELL, TRACK_ID, sep = "...")) %>% 
        mutate(UNIVERSAL_SPOT_ID              = paste(UNIVERSAL_TRACK_ID, FRAME, sep = "..."),
...
MatrixToTable <- function(x, df){eturn(df[,x])}
...
#     # Get file list
#     temp_file_list <- file_list %>% filter(image == image_list[FileX]) %>% as.data.table()
...
#       group_by(UNIVERSAL_SPOT_ID) %>% 
#       mutate(x = POSITION_X + 1, y = POSITION_Y + 1) %>% 
#       mutate(x_min = floor(x - r),
#              x_max = ceiling(x + r),
#              y_min = floor(y - r),
#              y_max = ceiling(y + r)) %>% 
#       mutate(test = (y_max < lim_y_max) & (x_max < lim_x_max) & (x_min > 0 & y_min > 0)) %>% 
...
#     TempTable <- TempTable %>% 
#       group_by(UNIVERSAL_TRACK_ID) %>% 
#       mutate(expand_x_min = (x-r) - x_min + 1,
#              expand_y_min = (y-r) - y_min + 1) %>% 
#       mutate(expand_x_min = round(expand_x_min*k),
#              expand_y_min = round(expand_y_min*k),
#              expand_x_max = expand_x_min + lim_r,
#              expand_y_max = expand_y_min + lim_r) %>% 
#       mutate(rows = y_max - y_min + 1,
#              cols = x_max - x_min + 1) %>% 
#       as.data.table()
    
#     # Get only select variables (for faster loops)
#     SplitTempTable <- TempTable %>% 
#       select(t, x_min, x_max, y_min, y_max, expand_x_min, expand_x_max, expand_y_min, expand_y_max, rows, cols) %>% 
#       arrange(t) %>% 
#       mutate(SpotID = 1:n()) %>% 
#       as_tibble() %>% group_split(t)
...
#     TempTable <- TempTable %>% select(-c(x, y, t, x_min, x_max, y_min, y_max, test, expand_x_min, expand_x_max, expand_y_min, expand_y_max, rows, cols)) %>% as.data.frame()
...
#       mutate(ABSOLUTE_POSITION_X = CELL_POSITION_X + POSITION_X,
#              ABSOLUTE_POSITION_Y = CELL_POSITION_Y + POSITION_Y) %>% 
#       mutate(ABSOLUTE_POSITION_X = ABSOLUTE_POSITION_X*CALIBRATION_UM,
...
ExtractFx <- function(FileX){
  tryCatch({
    ...
    if (nrow(temp_file_list) == 0) {
      stop("ERROR: temp_file_list is empty!")
    }

    # Get img metadata to exclude tracks that touch border
    print(paste("Reading metadata from:", temp_file_list$image))
    img_meta <- read_tags(temp_file_list$image)
    ...
    lim_x_max = img_meta$frame1$width
    lim_y_max = img_meta$frame1$length
    print(paste("Metadata read: lim_x_max =", lim_x_max, "lim_y_max =", lim_y_max))

    # Get cell data
    print(paste("Reading coordinates from:", temp_file_list$coordinates_output[1]))
    if (!file.exists(temp_file_list$coordinates_output[1])) {
      stop("ERROR: coordinates file does not exist!")
    }
    
    TempTable <- fread(temp_file_list$coordinates_output[1])
    print("Initial TempTable size:")
    print(dim(TempTable))
    ...
    print("TempTable after filtering:")
    print(dim(TempTable))

    if (nrow(TempTable) == 0) {
      stop("ERROR: TempTable is empty after filtering!")
    }

    # Read & split image
    N_FRAMES <- unique(TempTable$FRAME) + 1
    print(paste("Reading image with N_FRAMES =", N_FRAMES))
    
    img <- read_tif(temp_file_list$image, frames = N_FRAMES, msg = FALSE)
    img_frames <- lapply(1:NROW(N_FRAMES), split_img, img)

    if (length(img_frames) == 0) {
      stop("ERROR: img_frames is empty!")
    }
    ...
    # Ensure TempTable is not empty
    if (nrow(TempTable) == 0) {stop("ERROR: TempTable is empty before splitting!")}

    # Split table by 't' and ensure non-empty groups
    SplitTempTable <- TempTable %>%
      select(t, x_min, x_max, y_min, y_max, expand_x_min, expand_x_max, expand_y_min, expand_y_max, rows, cols) %>%
      arrange(t) %>%
      mutate(SpotID = 1:n()) %>%
    ...
    # Debugging: Print structure before applying lapply
    print(paste("SplitTempTable length before lapply:", length(SplitTempTable)))
    print(str(SplitTempTable))

    # Remove empty groups before converting to data.table
    SplitTempTable <- SplitTempTable[lengths(SplitTempTable) > 0]

    if (length(SplitTempTable) == 0) {stop("ERROR: All groups in SplitTempTable are empty!")}

    # Convert each element to data.table
    SplitTempTable <- lapply(SplitTempTable, as.data.table)

    # Pull intensities
    Intensities <- mapply(IntensityFx, SplitTempTable, img_frames, circle_mask_list, k, k2_r)

    if (is.null(Intensities)) {stop("ERROR: Intensities computation failed!")}

    print("Intensity computation successful.")

    # Save
    save_path <- paste0(temp_file_list$protein_path, "_intensity.csv.gz")
    suppressWarnings({file.remove(save_path, showWarnings = FALSE)})
    data.table::fwrite(TempTable, save_path, row.names = FALSE, na = "")

    print(paste("Saved intensities to:", save_path))

    return(save_path)
  }, error = function(e){
    print(paste("ERROR with ExtractFx FileX =", FileX))
    print(e)
  })
}
...
if(!file.exists(extraction_path)){dir.create(extraction_path)}
